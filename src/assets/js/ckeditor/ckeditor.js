...
(function() {
		...
		var a = {
			...
			domReady: function() {
				...
				return function(a) {
					...
					if (1 == d.length) if (document.addEventListener) document.addEventListener("DOMContentLoaded", b, !1),
	...
	CKEDITOR.event.prototype = function() {
		...
		b.prototype = {
			getListenerIndex: function(b) {
				for (var a = 0,
				d = this.listeners; a < d.length; a++) if (d[a].fn == b) return a;
				return - 1
			}
		};
		return {
	...
	CKEDITOR.env || (CKEDITOR.env = function() {
		...
		if (b.gecko) if (f < 10900) b.cssClass = b.cssClass + " cke_browser_gecko18";
		...
		CKEDITOR.tools = {
			...
			extend: function(b) {
				...
				for (var c = 1; c < a; c++) {
					var l = arguments[c],
					i;
					for (i in l) if (e === true || b[i] == void 0) if (!d || i in d) b[i] = l[i]
				}
				return b
			},
			...
			indexOf: function(b, a) {
				if (typeof a == "function") for (var e = 0,
				d = b.length; e < d; e++) {
					if (a(b[e])) return e
				} else {
			...
			createClass: function(b) {
				...
				c && this.extend(a.prototype, c, true);
			...
			convertRgbToHex: function(b) {
				return b.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi,
				function(b, a, d, c) {
					b = [a, d, c];
			...
			parseCssText: function(b, a, e) {
				...
				if (!b || b == ";") return d;
				b.replace(/&quot;/g, '"').replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g,
				function(b, e, c) {
					if (a) {
						e = e.toLowerCase();
			...
			objectCompare: function(b, a, c) {
				...
				if (!c) for (d in a) if (b[d] != a[d]) return false;
				return true
			},
	...
	CKEDITOR.dom.domObject.prototype = function() {
		...
		a.setCustomData = function(b, a) {
			var e = this.getUniqueId(); (c[e] || (c[e] = {}))[b] = a;
			return this
		};
		...
		clone: function(a, c) {
			...
			f = function(b) {
				...
				if (b.nodeType == CKEDITOR.NODE_ELEMENT) {
					...
					if (a) for (var b = b.childNodes,
					d = 0; d < b.length; d++) f(b[d])
				}
			};
			f(b);
	...
	CKEDITOR.dom.element.clearMarkers = function(a, c, b) {
		...
		function a(b) {
			for (var a = 0,
			e = 0,
			d = c[b].length; e < d; e++) a = a + (parseInt(this.getComputedStyle(c[b][e]) || 0, 10) || 0);
			return a
		}
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			removeClass: function(b) {
				...
				if (a) {
					b = RegExp("(?:^|\\s+)" + b + "(?=\\s|$)", "i");
					if (b.test(a))(a = a.replace(b, "").replace(/^\s+/, "")) ? this.setAttribute("class", a) : this.removeAttribute("class")
				}
				return this
			},
			hasClass: function(b) {
			...
			getClientRect: function() {
				...
				this.$.getBoundingClientRect()); ! b.width && (b.width = b.right - b.left); ! b.height && (b.height = b.bottom - b.top);
				return b
			},
			setHtml: function() {
				var b = function(b) {
			...
			getFirst: function(b) {
				var a = this.$.firstChild; (a = a && new CKEDITOR.dom.node(a)) && (b && !b(a)) && (a = a.getNext(b));
				return a
			},
			getLast: function(b) {
				var a = this.$.lastChild; (a = a && new CKEDITOR.dom.node(a)) && (b && !b(a)) && (a = a.getPrevious(b));
				return a
			},
			getStyle: function(b) {
			...
			moveChildren: function(b, a) {
				...
				if (c != b) {
					var d;
					if (a) for (; d = c.lastChild;) b.insertBefore(c.removeChild(d), b.firstChild);
					else for (; d = c.firstChild;) b.appendChild(c.removeChild(d))
				}
			},
			...
			removeAttributes: function(b) {
				if (CKEDITOR.tools.isArray(b)) for (var a = 0; a < b.length; a++) this.removeAttribute(b[a]);
				else for (a in b) b.hasOwnProperty(a) && this.removeAttribute(a)
			},
			removeStyle: function(b) {
				...
				if (!a.removeProperty && (b == "border" || b == "margin" || b == "padding")) {
					...
					for (var a = [], j = 0; j < c.length; j++) if (d) for (var l = 0; l < d.length; l++) a.push([b, c[j], d[l]].join("-"));
			...
			scrollIntoView: function(b) {
				...
				if (a) {
					do { (a.$.clientWidth && a.$.clientWidth < a.$.scrollWidth || a.$.clientHeight && a.$.clientHeight < a.$.scrollHeight) && !a.is("body") && this.scrollIntoParent(a, b, 1);
						if (a.is("html")) {
							var c = a.getWindow();
							try {
			...
			scrollIntoParent: function(b, a, c) {
				var d, j, l, i;
				function m(a, c) {
					if (/body|html/.test(b.getName())) b.getWindow().$.scrollBy(a, c);
					else {
			...
			copyAttributes: function(b, a) {
				...
				d = 0; d < c.length; d++) {
					...
					i;
					if (! (l in a)) if (l == "checked" && (i = this.getAttribute(l))) b.setAttribute(l, i);
			...
			renameNode: function(b) {
				if (this.getName() != b) {
					...
					b = new CKEDITOR.dom.element(b, a);
			...
			getChild: function() {
				...
				return function(a) {
					var c = this.$;
					if (a.slice) for (; a.length > 0 && c;) c = b(c, a.shift());
					else c = b(c, a);
	...
	!0, {
		...
		function a(a, b) {
			...
			if (this.current) d = this.current[f](false, u, g);
				...
				if (a) d.type == CKEDITOR.NODE_ELEMENT && (d = p > 0 ? d.getChild(p - 1) : g(d, true) === false ? null: d.getPreviousSourceNode(true, u, g));
				else {
					d = n;
	...
	CKEDITOR.dom.range = function(a) {
		...
		function c(a) {
			...
			return function(g) {
				return c(g) || b(g) ? true: !a && j(g) || g.type == CKEDITOR.NODE_ELEMENT && g.is(CKEDITOR.dtd.$removeEmpty)
			}
		}
		function b(a) {
			return ! l(a) && !i(a)
		...
		e = function(a, b, c, g) {
			...
			if (h.type == CKEDITOR.NODE_TEXT) {
				...
				e = true
			} else h = h.getChild(f).getPrevious();
			else {
		...
		CKEDITOR.dom.range.prototype = {
			...
			createBookmark2: function(a) {
				...
				if (a) {
					if (b.type == CKEDITOR.NODE_ELEMENT) {
						if ((f = b.getChild(g)) && f.type == CKEDITOR.NODE_TEXT && g > 0 && f.getPrevious().type == CKEDITOR.NODE_TEXT) {
							b = f;
							g = 0
						}
			...
			getBoundaryNodes: function() {
				...
				if (b.type == CKEDITOR.NODE_ELEMENT) {
					h = b.getChildCount();
					if (h > g) b = b.getChild(g).getPreviousSourceNode(true);
			...
			trim: function(a, b) {
				...
				c = this.endContainer;
				g = this.endOffset;
				if (!b && !h && c && c.type == CKEDITOR.NODE_TEXT) {
					if (g) {
			...
			enlarge: function(a, b) {
				...
				case CKEDITOR.ENLARGE_ELEMENT:
					...
					if (i.type == CKEDITOR.NODE_TEXT) {
						...
						q && (e = i.getChild(q - 1) || i.getLast());
						e || (k = i)
					}
					for (; k || e;) {
						if (k && !e) { ! j && k.equals(g) && (j = true);
						...
						for (; e;) {
							...
							if (e.type == CKEDITOR.NODE_COMMENT) e = e.getPrevious();
								...
								if (e.type == CKEDITOR.NODE_TEXT) {
									...
									q = e.getText();
									if (/[^\s\ufeff]/.test(q)) e = null;
									else for (var m = e.$.getElementsByTagName("*"), s = 0, A; A = m[s++];) if (!CKEDITOR.dtd.$removeEmpty[A.nodeName.toLowerCase()]) {
										e = null;
										break
									}
									e && (i = !!q.length)
			...
			shrink: function(a, b, c) {
				if (!this.collapsed) {
					...
					j = 1,
					i = 1;
					if (h && h.type == CKEDITOR.NODE_TEXT) if (e) if (e >= h.getLength()) g.setStartAfter(h);
					else {
						g.setStartBefore(h);
						j = 0
					} else g.setStartBefore(h);
					if (d && d.type == CKEDITOR.NODE_TEXT) if (f) if (f >= d.getLength()) g.setEndAfter(d);
					else {
						g.setEndAfter(d);
						i = 0
					...
					g.guard = function(b, g) {
						if (l(b)) return true;
						if (a == CKEDITOR.SHRINK_ELEMENT && b.type == CKEDITOR.NODE_TEXT || g && b.equals(m) || c === false && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary()) return false; ! g && b.type == CKEDITOR.NODE_ELEMENT && (m = b);
						return true
					};
					if (j)(h = g[a == CKEDITOR.SHRINK_ELEMENT ? "lastForward": "next"]()) && this.setStartAt(h, b ? CKEDITOR.POSITION_AFTER_START: CKEDITOR.POSITION_BEFORE_START);
					if (i) {
						g.reset(); (g = g[a == CKEDITOR.SHRINK_ELEMENT ? "lastBackward": "previous"]()) && this.setEndAt(g, b ? CKEDITOR.POSITION_BEFORE_END: CKEDITOR.POSITION_AFTER_END)
					}
					return ! (!j && !i)
				}
			},
			...
			splitBlock: function(a) {
				...
				if (g && g.equals(h)) if (b) {
					...
					h = this.splitElement(g); ! CKEDITOR.env.ie && !g.is("ul", "ol") && g.appendBogus()
				}
				return {
					previousBlock: g,
					nextBlock: h,
			...
			moveToElementEditablePosition: function(a, c) {
				...
				for (var d = 0; a;) {
					if (a.type == CKEDITOR.NODE_TEXT) {
						c && this.checkEndOfBlock() && m.test(a.getText()) ? this.moveToPosition(a, CKEDITOR.POSITION_BEFORE_START) : this.moveToPosition(a, c ? CKEDITOR.POSITION_AFTER_END: CKEDITOR.POSITION_BEFORE_START);
						d = 1;
						break
					}
					...
					h = d,
					e = void 0;
					g.type == CKEDITOR.NODE_ELEMENT && g.isEditable(false) && (e = g[c ? "getLast": "getFirst"](b)); ! h && !e && (e = g[c ? "getPrevious": "getNext"](b));
					a = e
				}
				return !! d
			},
	...
	CKEDITOR.SHRINK_TEXT = 2; (function() {
		...
		a.prototype = {
			getNextParagraph: function(a) {
				...
				if (!this._.nextNode) this._.nextNode = m || e.equals(n) || !n ? null: c(e, 1, n);
				return e
			}
		};
	...
	CKEDITOR.config = {
		...
		function h(a, b, c) {
			var d = a.name;
			if (D.$empty[d] || !a.children.length) if (d == "hr" && b == "br") a.replaceWith(p());
			else {
				a.parent && c.push({
					check: "it",
					el: a.parent
				...
				b: {
					...
					k; e < f; ++e) {
						k = d[e];
						if (k.type == CKEDITOR.NODE_ELEMENT && !h[k.name]) {
							h = false;
							break b
						}
					}
				...
				if (h) {
					...
					h = a.parent;
					for (var e = h.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || h.name == "body",
					j, f = d.length; f > 0;) {
						k = d[--f];
						if (e && (k.type == CKEDITOR.NODE_TEXT || k.type == CKEDITOR.NODE_ELEMENT && D.$inline[k.name])) {
							...
							j = null;
							k.insertAfter(a);
							h.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT && (k.type == CKEDITOR.NODE_ELEMENT && !D[h.name][k.name]) && c.push({
								check: "el-up",
								el: k
							})
						}
		...
		function w(a, b) {
			...
			for (h in d) if (h == "class") {
				c = d[h].split(/\s+/);
				for (f = a.classes.join("|"); k = c.pop();) if (f.indexOf(k) == -1) return false
		...
		CKEDITOR.filter.prototype = {
			allow: function(a, c, d) {
				...
				for (f = d.length; h < f; ++h) {
					...
					if (k.elements === true || k.elements === null) {
						...
						o = k.elements;
						delete k.elements;
						for (g in o) if (e[g]) e[g][u ? "unshift": "push"](k);
						else e[g] = [k]
					}
				}
				c.elements = e;
			...
			applyTo: function(a, b, c) {
				...
				for (; p = a.pop();) {
					...
					case "it":
						D.$removeEmpty[o.name] && !o.children.length ? h(o, c, a) : n(o) || h(o, c, a);
						break;
					case "el-up":
			...
			check: function(a, b, c) {
				...
				if (b !== false && (p = this._.transformations[g.name])) {
					for (h = 0; h < p.length; ++h) u(this, g, p[h]);
					m(g)
				}
		...
		return {
			load: function(b, f, e, d) {
				...
				if (l === 0) n(true);
					...
					var r = function(a, b) { (b ? i: m).push(a);
						if (--l <= 0) {
	...
	CKEDITOR.event.implementOn(CKEDITOR.ui); (function() {
		function a(a, d, g) {
			...
			if (d !== void 0) {
				...
				if (g == CKEDITOR.ELEMENT_MODE_INLINE && !d.is(CKEDITOR.dtd.$editable) || g == CKEDITOR.ELEMENT_MODE_REPLACE && d.is(CKEDITOR.dtd.$nonBodyContent)) throw Error('The specified element mode is not supported on element: "' + d.getName() + '".');
				this.element = d;
				this.elementMode = g;
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			...
			destroy: function(a) {
				this.fire("beforeDestroy"); ! a && r.call(this);
				this.editable(null);
			...
			setKeystroke: function() {
				...
				b = CKEDITOR.tools.isArray(arguments[0]) ? arguments[0] : [[].slice.call(arguments, 0)], c, d, g = b.length; g--;) {
					c = b[g];
					d = 0;
	...
	CKEDITOR.htmlParser = function() {
		...
		CKEDITOR.htmlParser.prototype = {
			...
			parse: function(b) {
				for (var f, e, d = 0,
				j; f = this._.htmlPartsRegex.exec(b);) {
					e = f.index;
					if (e > d) {
						d = b.substring(d, e);
						if (j) j.push(d);
					...
					if (j) j.push(f[0]);
						...
						if (!/="/.test(e)) {
							...
							this.onTagOpen(e, l, m); ! j && CKEDITOR.dtd.$cdata[e] && (j = [])
						}
	...
	CKEDITOR.htmlParser.fragment = function() {
				...
				if (a(e)) u.push(e);
					...
					if (d == "br") w.push(e);
						...
						for (;;) {
							...
							if (!e.isUnknown && !k.isUnknown && !j[d]) if (k.isOptionalClose) g.onTagClose(h);
							else if (d in b && h in b) {
								h = k.children; (h = h[h.length - 1]) && h.name == "li" || n(h = new CKEDITOR.htmlParser.element("li"), k); ! e.returnPoint && (e.returnPoint = k);
								k = h
							} else if (d in CKEDITOR.dtd.$listItem && !p(d, h)) g.onTagOpen(d == "li" ? "ul": "dl", {},
							0, 1);
							else if (h in c && !p(d, h)) { ! e.returnPoint && (e.returnPoint = k);
								k = k.parent
							} else {
				...
				return function(e) {
					if (e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						if (o) for (k(o, 1) && h.push(o); o;) {
							if (d(o) && (r = f(o)) && k(r)) if ((j = f(r)) && !d(j)) h.push(r);
							else {
								var u = r,
								l = g(p),
								y = u.parent.children,
						...
						for (o = 0; o < h.length; o++) i(h[o]);
						if (h = CKEDITOR.env.opera && !a || (typeof c == "function" ? c(e) !== false: c)) if (!p && CKEDITOR.env.ie && e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT) h = false;
				...
				return function(b) {
					if (b.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						if ("data-cke-bogus" in c || "data-cke-eol" in c) delete c["data-cke-bogus"];
							...
							var h = f(b); ! c && d(b.parent) ? l(b.parent, g(a)) : d(c) && (h && !d(h)) && j(c, g(a))
						}
					}
				}
			} (p);
		...
		function t(a, b) {
			for (var c = [], g = b.config.protectedSource, d = b._.dataStore || (b._.dataStore = {
				id: 1
			}), e = /<\!--\{cke_temp(comment)?\}(\d*?)--\>/g, g = [/<script[\s\S]*?<\/script>/gi, /<noscript[\s\S]*?<\/noscript>/gi].concat(g), a = a.replace(/<\!--[\s\S]*?--\>/g,
			function(a) {
				return "<\!--{cke_tempcomment}" + (c.push(a) - 1) + "--\>"
			}), h = 0; h < g.length; h++) a = a.replace(g[h],
			function(a) {
				a = a.replace(e,
				function(a, b, g) {
					return c[g]
		...
		CKEDITOR.htmlDataProcessor = function(b) {
			...
			b.on("toDataFormat",
			function(a) {
				a.data.dataValue = CKEDITOR.htmlParser.fragment.fromHtml(a.data.dataValue, b.editable().getName(), c(b.config))
			},
			null, null, 5);
			b.on("toDataFormat",
			function(a) {
		...
		CKEDITOR.htmlDataProcessor.prototype = {
			toHtml: function(a, b, c, g) {
				var d = this.editor; ! b && b !== null && (b = d.editable().getName());
		...
		function f(a) {
			...
			if (a.elementMode == CKEDITOR.ELEMENT_MODE_REPLACE && b.is("textarea")) {
				...
				if (c) {
					...
					c.on("submit", e);
					if (!c.$.submit.nodeName && !c.$.submit.length) c.$.submit = CKEDITOR.tools.override(c.$.submit,
					function(b) {
						return function() {
							a.updateElement();
	...
	CKEDITOR.config.startupMode = "wysiwyg"; (function() {
		...
		function e(a) {
			...
			if (d.contains(e)) {
				...
				d = function(a, g) {
					return function(a, d) {
						d && (a.type == CKEDITOR.NODE_ELEMENT && a.is(e)) && (c = a);
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				insertElement: function(a) {
					...
					e = c.getSelection(), f = e.getRanges(), i = a.getName(), k = CKEDITOR.dtd.$block[i], m, n, l, B = f.length - 1; B >= 0; B--) {
						...
						if (!m.checkReadOnly()) {
							...
							n = !B && a || a.clone(1);
							var q, z;
							if (k) for (; (q = m.getCommonAncestor(0, 1)) && (z = CKEDITOR.dtd[q.getName()]) && (!z || !z[i]);) if (q.getName() in CKEDITOR.dtd.span) m.splitElement(q);
					...
					if (l) {
						...
						if (k) if ((a = l.getNext(b)) && a.type == CKEDITOR.NODE_ELEMENT && a.is(CKEDITOR.dtd.$block)) a.getDtd()["#"] ? m.moveToElementEditStart(a) : m.moveToElementEditEnd(l);
				...
				setup: function() {
					...
					function() {
						var b = this.getData();
						this.is("textarea") || a.config.ignoreEmptyParagraph !== false && (b = b.replace(l,
						function(a, b) {
							return b
						}));
						a.setData(b, null, 1)
					...
					if (!this.is("textarea")) {
						...
						g = b.getCustomData("stylesheet_ref") || 0;
						...
						function(b) {
							...
							if (c in {
								...
								if (d = e(d)) {
									...
									a.fire("saveSnapshot");
									g = 1
								} else if (h.blockLimit && h.blockLimit.is("td") && (p = h.blockLimit.getAscendant("table")) && b.checkBoundaryOfElement(p, c ? CKEDITOR.START: CKEDITOR.END) && (j = p[c ? "getPrevious": "getNext"](i))) {
									a.fire("saveSnapshot");
									b["moveToElementEdit" + (c ? "End": "Start")](j);
		...
		var n = function() {
			...
			return function(m, q, t) {
				...
				if (!A.checkReadOnly()) {
					...
					x = s.createBookmark(); (o = x.startNode.getPrevious(b)) && a(o) && e(o) && v.push(o); (o = x.startNode.getNext(b)) && a(o) && e(o) && v.push(o);
					...
					if (t) {
						...
						for (i(I, E); x < I.length; x++) {
							...
							if (y) v = x > 0;
								...
								if (G.lastNotAllowed && x < I.length - 1) { (O = Q ? o: O) && E.setEndAt(O, CKEDITOR.POSITION_AFTER_START);
									s = 0
								}
								E.collapse()
							}
			...
			function() {
				...
				if (CKEDITOR.env.ie && !p) {
					...
					if (b.$.compatMode != "BackCompat") {
						if (CKEDITOR.env.ie7Compat || CKEDITOR.env.ie6Compat) j.on("mousedown",
						function(a) {
							function b(a) {
								a = a.data.$;
								if (d) {
			...
			getRanges: function() {
				...
				function() {
					...
					var b = function(b, c) {
						...
						h, i, j = b.duplicate(), m = 0, p = f.length - 1, l = -1, n, o; m <= p;) {
							l = Math.floor((m + p) / 2);
							h = f[l];
				...
				return function(b) {
					...
					e = 0; e < d.length; e++) {
						...
						if (!f.collapsed) {
							...
							m = f.endOffset,
							l = f.clone();
							b && b.type == CKEDITOR.NODE_TEXT && (j >= b.getLength() ? l.setStartAfter(b) : l.setStartBefore(b));
				...
				if (a.length) if (this.isLocked) {
					...
					if (n) {
						...
						if (l && l.type == CKEDITOR.NODE_ELEMENT && l.getName() in r && (!l.is("a") || !l.getText())) try {
							...
							m.select();
							return
						} catch(B) {} (b.startContainer.type == CKEDITOR.NODE_ELEMENT && b.startContainer.getName() in i || b.endContainer.type == CKEDITOR.NODE_ELEMENT && b.endContainer.getName() in i) && b.shrink(CKEDITOR.NODE_ELEMENT, true);
						m = b.createBookmark();
						var i = m.startNode,
						q;
						if (!a) q = m.endNode;
						...
						if (q) {
							...
							k = !(k && k.getText && k.getText().match(e)) && (j || !i.hasPrevious() || i.getPrevious().is && i.getPrevious().is("br"));
	...
	CKEDITOR.STYLE_OBJECT = 3; (function() {
		...
		function c(b) {
			...
			if (b.collapsed) {
				...
				for (l.getPosition(m) == CKEDITOR.POSITION_FOLLOWING && (l = 0); l;) {
					...
					if (l.equals(m)) {
						...
						if (!s || i[s] && !t && (!r || h) && (l.getPosition(m) | CKEDITOR.POSITION_PRECEDING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED) == CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED && (!e.childRule || e.childRule(l))) {
							...
							if (v && ((v.getDtd() || CKEDITOR.dtd.span)[d] || f) && (!e.parentRule || e.parentRule(v))) {
								if (!n && (!s || !CKEDITOR.dtd.$removeEmpty[s] || (l.getPosition(m) | CKEDITOR.POSITION_PRECEDING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED) == CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED)) {
									n = b.clone();
									n.setStartBefore(l)
								}
								if (q == CKEDITOR.NODE_TEXT || r || q == CKEDITOR.NODE_ELEMENT && !l.getChildCount()) {
									for (var q = l,
									w; (g = !q.getNext(A)) && (w = q.getParent(), i[w.getName()]) && (w.getPosition(k) | CKEDITOR.POSITION_FOLLOWING | CKEDITOR.POSITION_IDENTICAL | CKEDITOR.POSITION_IS_CONTAINED) == CKEDITOR.POSITION_FOLLOWING + CKEDITOR.POSITION_IDENTICAL + CKEDITOR.POSITION_IS_CONTAINED && (!e.childRule || e.childRule(w));) q = w;
									n.setEndAfter(q)
								}
							} else g = true
						} else g = true;
					...
					if (g && n && !n.collapsed) {
						for (var g = u(this, c), r = g.hasAttributes(), t = n.getCommonAncestor(), q = {},
						s = {},
						v = {},
						D = {},
						B, z, F; g && t;) {
		...
		function b(a) {
			...
			c = b.startNode;
			if (a.collapsed) {
				for (var d = new CKEDITOR.dom.elementPath(c.getParent(), a.root), e, f = 0, h; f < d.elements.length && (h = d.elements[f]); f++) {
		...
		function f(a) {
			var b = a.getEnclosedNode() || a.getCommonAncestor(false, true); (a = (new CKEDITOR.dom.elementPath(b, a.root)).contains(this.element, 1)) && !a.isReadOnly() && w(a, this)
		}
		function e(a) {
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.text.prototype, {
		...
		function a(a, c, e) {
			...
			c[e ? "endContainer": "startContainer"] = j;
			return c
		}
					...
					getNextRange: function(j) {
						...
						if (l && a.length > 1) {
							...
							if (j) for (var m = 0; a[d + m + 1];) {
								...
								j = 0,
								i = n.getById(e[m].endNode), n = n.getById(e[m + 1].startNode);;) {
...
*/
				...
				select: function(b, a, d) {
					if (! (3 > arguments.length)) {
						...
						function() {
							...
							p(c);
							for (var k = 0,
							j; k < a.items.length && (j = a.items[k]); k++) i.push('<option value="', CKEDITOR.tools.htmlEncode(j[1] !== void 0 ? j[1] : j[0]).replace(/"/g, "&quot;"), '" /> ', CKEDITOR.tools.htmlEncode(j[0]));
							if (typeof c.inputStyle != "undefined") c.style = c.inputStyle;
	...
	CKEDITOR.DIALOG_RESIZE_BOTH = 3; (function() {
		...
		CKEDITOR.dialog = function(a, b) {
			...
			function d(b) {
				if (l == CKEDITOR.dialog._.currentTop) {
					...
					l._.tabBarMode ? (c = c ? p.call(l) : u.call(l), l.selectPage(c), l._.tabs[c][0].focus()) : e(c ? -1 : 1),
					o = 1;
					else if (c == CKEDITOR.ALT + 121 && !l._.tabBarMode && 1 < l.getPageCount()) l._.tabBarMode = !0,
					l._.tabs[l._.currentTabId][0].focus(),
					o = 1;
					else if ((37 == c || 39 == c) && l._.tabBarMode) c = c == (d ? 39 : 37) ? p.call(l) : u.call(l),
					l.selectPage(c),
					l._.tabs[c][0].focus(),
					o = 1;
			...
			this.on("hide",
			function() {
				v.removeListener("keydown", d); (CKEDITOR.env.opera || CKEDITOR.env.gecko) && v.removeListener("keypress", g);
				C(function(a) {
					Q.apply(a)
				})
			});
		...
		M = function(a) {
			...
			if ((b = t[(b ? "CTRL+": "") + (c ? "ALT+": "") + (e ? "SHIFT+": "") + d]) && b.length) b = b[b.length - 1],
		...
		O = function(a, b, c, e, d) { (t[c] || (t[c] = [])).push({
				uiElement: a,
				dialog: b,
				key: c,
		...
		function w(a) {
			...
			function j(d, b) {
				...
				if (!c.getById("cke_pastebin")) {
					...
					var g = j.select();
					if (CKEDITOR.env.ie) var k = e.once("blur",
					function() {
						a.lockSelection(g)
					});
			...
			a.getClipboardData = function(b, c) {
				...
				null, null, 1E3); ! 1 === o() && (a.removeListener("paste", e), f && a.fire("pasteDialog", j) ? (a.on("pasteDialogCommit", l), a.on("dialogHide",
				function(a) {
					a.removeListener();
	...
	CKEDITOR.config.toolbarLocation = "top"; (function() {
		...
		p.prototype = {
			...
			exec: function(d) {
				function i(n) {
					for (var j = l.startContainer,
					b = l.endContainer; j && !j.getParent().equals(n);) j = j.getParent();
					for (; b && !b.getParent().equals(n);) b = b.getParent();
					if (j && b) {
						for (var c = j,
						j = [], a = !1; ! a;) c.equals(b) && (a = !0),
						j.push(c),
						c = c.getNext();
						if (! (1 > j.length)) {
							c = n.getParents(!0);
							...
							if (h && h.length) for (b = 0; b < h.length; b++) {
								for (g = n = h[b]; (g = g.getNext()) && g.is && g.getName() in m;) CKEDITOR.env.ie && !n.getFirst(function(b) {
									return t(b) && u(b)
								}) && n.append(l.document.createText(" ")),
								n.append(g);
								n.insertAfter(i)
							}
						}
				...
				for (var e = this,
				o = {},
				g = d.getSelection(), v = g.createBookmarks(1), l, p = (g && g.getRanges(1)).createIterator(); l = p.getNextRange();) {
					...
					if (h) {
						var k = h.getFirst(q),
						w = !!k.getNext(q),
						s = l.startContainer; (!k.equals(s) && !k.contains(s) || !("indent" == e.name || e.useIndentClasses || parseInt(h.getStyle(r(h)), 10)) || !f(h, !w && k.getDirection())) && i(h)
					} else a()
				}
		...
		CKEDITOR.plugins.add("indent", {
			requires: "list",
			onLoad: function() { (CKEDITOR.env.ie6Compat || CKEDITOR.env.ie7Compat) && CKEDITOR.addCss(".cke_editable ul,.cke_editable ol{\tmargin-left: 0px;\tpadding-left: 40px;}")
			},
			init: function(d) {
				...
				if (b = b || p(a)) {
					...
					if (l) {
						...
						if (!j && !h) {
							...
							if (CKEDITOR.env.ie && j && (!h || !d.getChildCount())) b.moveToElementEditStart(h ? d: e),
							b.select();
	...
	CKEDITOR.htmlWriter = CKEDITOR.tools.createClass({
		...
		function n(a) {
			var c = this.editor,
			d = a.document,
			b = d.body; (a = d.getElementById("cke_actscrpt")) && a.parentNode.removeChild(a); (a = d.getElementById("cke_shimscrpt")) && a.parentNode.removeChild(a);
			...
			this.setup();
			CKEDITOR.env.ie && (d.getDocumentElement().addClass(d.$.compatMode), c.config.enterMode != CKEDITOR.ENTER_P && d.on("selectionchange",
			function() {
				var a = d.getBody(),
				b = c.getSelection(),
				e = b && b.getRanges()[0];
			...
			} (CKEDITOR.env.gecko || CKEDITOR.env.ie && "CSS1Compat" == c.document.$.compatMode) && this.attachListener(this, "keydown",
			function(a) {
				var b = a.data.getKeystroke();
		...
		function o(a) {
			if (!a.readOnly) {
				...
				if (!f || 1 == f && e.type == CKEDITOR.NODE_ELEMENT && e.hasAttribute("_moz_editor_bogus_node")) {
					...
					d.$.dispatchEvent(j); (h != g.$.scrollTop || i != g.$.scrollLeft) && e.getWindow().$.scrollTo(i, h);
					f && b.getFirst().remove();
		...
		k = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				detach: function() {
					...
					d.clearCustomData();
					CKEDITOR.tools.removeFunction(this._.frameLoadedHandler); (c = d.removeCustomData("onResize")) && c.removeListener();
	...
	CKEDITOR.tools.extend(CKEDITOR.config, {
		...
		function ca(a) {
			...
			p(d, {
				...
				look: function(a) {
					if (this.oldLook != a) {
						for (var d = this.lineChildren.length,
						c; d--;)(c = this.lineChildren[d]).setAttribute("style", c.base + c.looks[0 | a / 2]);
						this.oldLook = a
					}
				},
		...
		function W(a) {
			...
			if (o(f) || ea(a, f, i) || f.getParent().is(X)) return null;
			var h = q(a, f, !i);
			if (h) {
			...
			} else f.equals(a.editable[i ? "getLast": "getFirst"](a.isRelevant)) ? (B(a), i && b.y > f.size.bottom - e && b.y < d.pane.height && f.size.bottom > d.pane.height - e && f.size.bottom < d.pane.height ? j = x: 0 < b.y && b.y < f.size.top + e && (j = w)) : j = r,
			c = [null, f][i ? "reverse": "concat"]().concat([i ? V: C, F, j, f.equals(a.editable[i ? "getLast": "getFirst"](a.isRelevant)) ? i ? x: w: r]);
		...
		function M(a, b, d, e) {
			...
			} (), c = b.getDocumentPosition(), j = {},
			i = {},
			h = {},
			k = {},
			g = s.length; g--;) j[s[g]] = parseInt(f("border-" + s[g] + "-width"), 10) || 0,
			h[s[g]] = parseInt(f("padding-" + s[g]), 10) || 0,
			i[s[g]] = parseInt(f("margin-" + s[g]), 10) || 0; (!d || e) && G(a, e);
		...
		function ga(a, b, d, e) {
			...
			i = !1,
			h = !1,
			k = a.view.pane.height,
			g = a.mouse; g.y + j < k && 0 < g.y - j;) {
				i || (i = b(f, e));
				h || (h = b(c, e)); ! i && 0 < g.y - j && (f = d(a, {
					x: g.x,
					y: g.y - j
				})); ! h && g.y + j < k && (c = d(a, {
					x: g.x,
			...
			init: function(a) {
				...
				function() {
					...
					b.is(v.$inline) || (c.inInlineMode && !T(b) && b.setStyles({
						...
						top: null,
						left: null
					}), ca.call(this, c), G(c), b.attachListener(a, "beforeUndoImage",
					function() {
						c.line.detach()
					...
					function(b) {
						if ("wysiwyg" == a.mode) if (c.inInlineMode) {
							...
							f = c.view.scroll;
							if (! (d > e.left - f.x && d < e.right - f.x) || !(b > e.top - f.y && b < e.bottom - f.y)) clearTimeout(h),
							h = null,
							c.line.detach()
						} else clearTimeout(h),
						h = null,
					...
					function(b) {
						...
						if (! ("wysiwyg" != a.mode || a.readOnly || h)) {
							...
							h = setTimeout(function() {
								...
								h = c.trigger = null;
								G(c);
								if (i && !c.hiddenMode && a.focusManager.hasFocus && !c.line.mouseNear() && (c.element = Z(c, !0)))(c.trigger = U(c) || W(c) || $(c)) ? c.line.attach().place() : (c.trigger = null, c.line.detach()),
								i = !1
							},
							30)
						}
					...
					function() {
						c.hiddenMode = 0
					}), a.addCommand("accessPreviousSpace", R(c)), a.addCommand("accessNextSpace", R(c, !0)), a.setKeystroke([[d.magicline_keystrokePrevious, "accessPreviousSpace"], [d.magicline_keystrokeNext, "accessNextSpace"]]), a.on("loadSnapshot",
					function() {
		...
		$ = function() {
			function a(a) {
				...
				i, h, k, g; c && !c.equals(f) && (h = f.getNext(a.isRelevant));) i = Math.abs(ba(a, f, h) - a.mouse.y),
				i < e && (e = i, k = f, g = h),
				f = h,
				m(a, f);
	...
	CKEDITOR.plugins.pagebreakCmd = {
		...
		var h, j = {
			...
			exec: function(a) {
				...
				c = "";
				i && (window._cke_htmlToLoad = a.dataValue, c = 'javascript:void( (function(){document.open();document.domain="' + document.domain + '";document.write( window.opener._cke_htmlToLoad );document.close();window.opener._cke_htmlToLoad = null;})() )');
	...
	CKEDITOR.tools.buildTableMap = function(p) {
		...
		CKEDITOR.plugins.add("undo", {
			init: function(a) {
				...
				function() {
					...
					function(a) { ! a.data.$.ctrlKey && !a.data.$.metaKey && b.type(a)
					})
				});
				a.on("beforeModeUnload",
				function() {
		...
		h.prototype = {
			equals: function(a, c) {
				var d = this.contents,
				b = a.contents;
				if (CKEDITOR.env.ie && (CKEDITOR.env.ie7Compat || CKEDITOR.env.ie6Compat)) d = d.replace(j, ""),
				b = b.replace(j, "");
				if (d != b) return ! 1;
				if (c) return ! 0;
				d = this.bookmarks;
		...
		i.prototype = {
			...
			getNextImage: function(a) {
				...
				if (d) if (a) for (b = this.index - 1; 0 <= b; b--) {
					if (a = c[b], !d.equals(a, !0)) return a.index = b,
					a
				} else for (b = this.index + 1; b < c.length; b++) if (a = c[b], !d.equals(a, !0)) return a.index = b,
				a;
				return null
			},
			redoable: function() {
